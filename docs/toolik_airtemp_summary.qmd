...
```{r}
toolik_clorophyll <- read_csv(here::here("data", "data-raw", "toolik_chlorophyll.csv"))

names(toolik_clorophyll)
skimr::skim(toolik_clorophyll)

toolik_clorophyll <- toolik_clorophyll %>%
  clean_names()
dim(toolik_clorophyll)
names(toolik_clorophyll)
```


```{r}
toolik_clorophyll$active_chl_a_ug_l <- na_if(toolik_clorophyll$active_chl_a_ug_l, "."  )

toolik_clorophyll$active_chl_a_ug_l <- as.numeric(toolik_clorophyll$active_chl_a_ug_l)
```

```{r}
ggplot(data = toolik_clorophyll,
       aes(x = depth, y = active_chl_a_ug_l))+
  geom_point(aes(color = active_chl_a_ug_l),
             show.legend = FALSE) + 
  scale_color_gradient(low = "blue", high = "orange") +
  theme_minimal()
# theme minimal removed the grey background of the plot ----
